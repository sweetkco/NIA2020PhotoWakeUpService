...
class SMPL():
  ...
  def set_params(self, pose=None, beta=None, trans=None):
    ...
    if trans is not None:
      self.trans = trans
    self.update()
    return self.verts, self.J, self.weights
  ...
  def update(self):
    ...
    print('Self joints : {}'.format(self.J.shape))
    ...
    self.J = self.J_regressor.dot(v_posed)
    print(self.J.shape)
    print(self.J_regressor.shape)
    print("-"*90)
    # transformation of each vertex
    T = np.tensordot(self.weights, G, axes=[[1], [0]])
    #self.J = np.matmul(self.J.reshape(-1,1,3),self.R).reshape(-1,3)
    #print(self.J.shape)
  ...
  def save_keypoints(self,name):
    print("SD")
    np.save(os.path.join(self.out_path, '{}_joints.npy'.format(name)), self.J)
