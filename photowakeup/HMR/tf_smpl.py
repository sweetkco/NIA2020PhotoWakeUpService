...
class SMPL(object):
    ...
    def __call__(self, beta, theta, get_skin=False, name=None, get_weights=False):
        ...
        with tf.name_scope(name, "smpl_main",values= [beta, theta]):
            ...
            joint_x = tf.matmul(verts[:, :, 0], self.J_regressor)#self.joint_regressor)
            joint_y = tf.matmul(verts[:, :, 1], self.J_regressor)#self.joint_regressor)
            joint_z = tf.matmul(verts[:, :, 2], self.J_regressor)#self.joint_regressor)
            joints = tf.stack([joint_x, joint_y+0.2, joint_z], axis=2)
            print("tf_joints {}".format(joints.shape))
            print(self.joint_regressor.shape)
            print("-"*100)

            return verts, joints,W
