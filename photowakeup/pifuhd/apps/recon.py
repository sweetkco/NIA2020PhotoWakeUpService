...
def gen_mesh(res, net, cuda, data, save_path, thresh=0.5, use_octree=True, components=False):
    ...
    try:
        ...
        cv2.imwrite(save_img_path, save_img_list[0])
        '''
...
def gen_mesh_imgColor(res, net, cuda, data, save_path, thresh=0.5, use_octree=True, components=False):
    ...
    try:
        save_img_path = save_path[:-4] + '.png'
        save_img_path_texture = save_path[:-4]+'.jpg'
        ...
        #copied_image_tensor = image_tensor.clone().detach()
        #save_img = (np.transpose(copied_image_tensor.squeeze(0).cpu().numpy(),(1,2,0))*0.5+0.5)[:, :, ::-1]*255.0
        #cv2.imwrite(save_img_path_texture,save_img)
...
def recon(opt, use_rect=False):
    ...
    if opt.pifu_checkpoint is not None:
        state_dict_path = opt.pifu_checkpoint
